...
BUILDER_DRAFT_PATH = (
    ROOT.parent / "persona-builder" / "golden" / "boundary-setter-input.json"
)
    ...
    for signal in signals:
        ...
        if state == "baseline":
            ...
            if event in {"candidate_committed", "candidate_blocked"}:
                if (
                    record["influenced_turns"]
                    >= policy["max_consecutive_influenced_turns"]
                ):
                ...
                if (
                    record["unreinforced_turns"]
                    >= policy["decay_after_unreinforced_turns"]
                ):
                    record["state"] = "recovering"
            ...
            state == "recovering"
            and event == "turn_close"
            and signal["recovery_completed"]
        ):
...
def validate(program: dict[str, Any]) -> None:
    ...
    for scenario in program["rehearsal_scenarios"]:
        ...
        if initial["activation_count"] > policy["max_activations_per_scene"]:
            raise ValueError(
                f"initial activation count exceeds policy: {scenario['scenario_id']}"
            )
        event_ids: set[str] = set()
        for sequence, signal in enumerate(scenario["signals"], start=1):
            if signal["sequence"] != sequence:
                raise ValueError(
                    f"noncontiguous signal sequence: {scenario['scenario_id']}"
                )
            ...
            if len(signal["trigger_ids"]) != len(set(signal["trigger_ids"])):
                raise ValueError(
                    f"duplicate trigger reference: {scenario['scenario_id']}"
                )
            if not set(signal["trigger_ids"]) <= trigger_ids:
                raise ValueError(
                    f"unknown trigger reference: {scenario['scenario_id']}"
                )
            if signal["trigger_ids"] != sorted(signal["trigger_ids"]):
                raise ValueError(
                    f"trigger references not sorted: {scenario['scenario_id']}"
                )
...
def migrate_builder_draft_v01(
    source: dict[str, Any], program: dict[str, Any]
) -> dict[str, Any]:
...
def verify_state_machine(program: dict[str, Any]) -> None:
    ...
    if not required <= scenarios.keys():
        raise ValueError(
            f"missing state-machine scenarios: {sorted(required - scenarios.keys())}"
        )
    ...
    recurrence_states, recurrence_final = reduce_scenario(
        scenarios["recurrence-bound"], policy
    )
    if (
...
def verify_projection_and_migration(
    program: dict[str, Any], actual: dict[str, Any]
) -> None:
    ...
    if any(program["forbidden_effects"].values()):
        raise ValueError(
            "character program gained a forbidden capability or safety effect"
        )
    ...
    if set(migrated) != set(source) | {"character_program"}:
        raise ValueError(
            "migration changed the closed root by more than character_program"
        )
    ...
    migrated_controls = {
        key: migrated["behavior"][key]
        for key in ("directness", "warmth", "humor", "disagreement")
    }
    expected_controls = {
...
def render_agents(p: dict[str, Any]) -> str:
    ...
    lines.extend(
        ["", "Rationalizations:", *bullets(f["rationalizations"]), "", "Escalation:"]
    )
    ...
    for item in p["risk_appetite"]:
        lines.append(
            f"- `{item['domain']}` ({item['appetite']}): {item['decision_rule']}"
        )
    ...
    for key in (
        ...
        lines.append(
            f"- {key.replace('_', ' ').title()}: {p['pressure_behavior'][key]}"
        )
    return "\n".join(lines) + "\n"
...
def render_relationships(p: dict[str, Any]) -> str:
    ...
    for item in p["relationship_hooks"]:
        lines.extend(
            [f"- `{item['id']}`: {item['cue']}", f"  Response: {item['response']}"]
        )
...
def render_scenarios(p: dict[str, Any]) -> str:
    ...
    for scenario in p["rehearsal_scenarios"]:
        ...
        for signal, state in zip(
            scenario["signals"], scenario["expected_states"], strict=True
        ):
...
def main() -> int:
    ...
    parser.add_argument(
        "--write", action="store_true", help="replace the golden projection"
    )
    ...
    if encoded != expected:
        raise SystemExit(
            "golden projection mismatch; inspect changes and use --write deliberately"
        )

...
[ADR 0003](../adr/0003-portable-characters-and-community-library.md). It defines
authored character semantics and a deterministic projection, not a second pack
format, archive, validator, installer, publisher, or catalog. Where this document
describes future implementation, ADR 0003 and the current
[Persona Pack Specification](../PERSONA-PACK-SPEC.md) control the portable
...
Five versions remain independent:
...
fields, four runtime destinations, and the provenance metadata role. It creates no file, role, tool,
...
The production authoring path is original-character-first. The Character Wizard
MAY offer this contract for original private characters, but the repository's
researched historical cast is a distinct set of prebuilt, source-informed
educational interpretations. A historical candidate is not made official by
projection, strict validation, local presence, or runtime use. The default
wizard, prebuilt cast, and any project-operated catalog MUST NOT provide
copyrighted entertainment characters, copied scripts, actor images, cloned
voices, or unreviewed likenesses. Rights and catalog admission remain the
separate review decisions defined by ADR 0003 and the
[content boundaries](../CONTENT-BOUNDARIES.md).

## Integration boundaries

Character programs and `.greenroom` packs are declarative, non-executable data.
They do not grant shell, filesystem, browser, external messaging, plugin, tool,
credential, provider-selection, or network authority. The host owns every
capability allowlist and defaults all such capabilities to absent. A pack MUST
NOT contain a provider credential, secret, arbitrary endpoint, request header,
room transcript, memory record, or character-state record.

Provider and memory identifiers, if a future runtime needs them, are opaque,
non-secret references stored and resolved by the host authority outside the
pack. They never confer authority, select a provider endpoint, reveal a
credential, or become catalog metadata. In invited rooms, the room owner's local
companion remains the authority for membership, durable state, memory, provider
resolution, and model calls; character content cannot act as a human invitation
or membership credential.

The lifecycle operations are deliberately distinct:

1. **Validate** checks hostile archive structure and exact pack semantics without
   extraction or execution.
2. **Inspect** runs that same strict gate and exposes the exact immutable persona
   segment; it does not import, install, persist, or call a provider.
3. **Install** is a future explicit local-authority operation over the exact
   inspected bytes and digest; validator success alone does not install.
4. **Publish** is a separately reviewed release action; local creation,
   validation, inspection, or installation never publishes.
5. **Catalog admission** is a version-and-digest-specific trust and rights
   decision under accepted catalog/community policy; it is not syntax
   validation, publication, popularity, or local runtime use.
...
The projection oracle exercises reducer transitions over one complete rehearsal
signal stream only. It is not the future durable reducer and does not claim to
persist `last_event_sequence` or event-ID/digest evidence across calls. A runtime
implementation must add that authority-owned durable record and the cross-call
replay/reuse tests required below before claiming this state contract.
...
The complete Persona Pack runtime segment still uses the specification's exact
order: `AGENTS.md`, `BACKGROUND.md`, `VOICE.md`, then optional
`RELATIONSHIPS.md` and `SCENARIOS.md`. `persona.yaml`, `PROVENANCE.md`, optional
`SOURCES.md`, `LICENSE`, assets, validator reports, catalog records, memory, and
state are never model-visible. The projection oracle emits deterministic file
fragments for design verification; it is not the strict archive validator and
does not make its output loadable, installed, published, or catalog-admitted.
Only a complete canonical `.greenroom` archive that passes `greenroom-persona`
may proceed to a future explicit installer.
...
`uv run python docs/programmable-character/verify_contract.py` MUST pass before fixture

...
SPEC = importlib.util.spec_from_file_location(
    "verify_contract", ROOT / "verify_contract.py"
)
...
class AuthoredStringValidationTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.program = json.loads(
            verify_contract.PROGRAM_PATH.read_text(encoding="utf-8")
        )
    ...
    def test_schema_rejects_control_and_unicode_noncharacters(self) -> None:
        for value in (
            ...
            with self.subTest(value=ascii(value)):
                self.assertTrue(
                    list(self.schema_validator.iter_errors(self.mutated(value)))
                )
    ...
    def test_golden_program_remains_valid_and_projectable(self) -> None:
        ...
        self.assertEqual(
            verify_contract.project(self.program)["projection_version"], "0.1"
        )

...
Each IPC pair receives a unique random 256-bit capability: `provider_ipc_hmac`, `director_ipc_hmac`, `lantern_ipc_hmac`, or `harbor_ipc_hmac`. `scripts/bootstrap_local_identity.py` generates the capability files without printing them, stores host sources mode `0400` under the gitignored private run directory, and Compose mounts each as a read-only secret at `/run/secrets/ipc_hmac`. A capability is mounted into exactly `supervisor` and its one peer. The provider API credential is mounted only into `provider`; each identity private key is mounted only into its corresponding signer; neither class of operational secret is mounted into `supervisor`, `ipc-init`, Buzz, or a sibling. Compose-local file-backed secret modes and the actual service mount lists are independently inspected at the Compose gate; an implementation that exposes a secret through environment, argv, image layers, Compose interpolation, logs, or another service fails the gate.
...
```json
{"v":1,"key_id":"<rotation-id>","caller":"supervisor","peer":"provider|signer-director|signer-lantern|signer-harbor","operation":"ready|generate|recover|sign_auth|sign_room_event|cancel","request_id":"<32 lowercase hex>","nonce":"<64 lowercase hex>","issued_at_ms":1788100000000,"expires_at_ms":1788100005000,"payload":{},"mac":"<64 lowercase hex>"}
```

No extra or missing fields are accepted. `mac = HMAC-SHA256(capability, "greenroom-ipc-request-v1\n" || canonical_json(request_without_mac))`. The peer verifies the configured caller/peer/operation allowlist, current `key_id`, constant-time MAC equality, `expires_at_ms >= issued_at_ms`, a validity interval no greater than 5 seconds, wall-clock skew no greater than 5 seconds, and unique `(key_id, request_id, nonce)` before parsing operation payload or doing cryptographic/provider/network work. Accepted triples and request digests are committed to the peer's private durable journal before the operation starts and survive service restart; a replay is rejected even if bytes and MAC are valid. An application retry uses a fresh request ID/nonce but the same durable `decision_id` and canonical operation hash, allowing the existing provider/signer idempotency journal to return the one prior result without repeating paid or cryptographic work.

Operation payloads are exact and size-bounded:

- `ready`: `{}`; returns service identity, signer public key where applicable, policy digest, current `key_id`, and journal readiness.
...
Capabilities rotate by adding a new Compose secret and `key_id`, briefly mounting current and next keys only into the same pair, recreating the peer, completing authenticated readiness on the new ID, recreating `supervisor`, and then removing the old mount in a second reviewed Compose change. The overlap is capped at 60 seconds; requests use only the new key after cutover, and the peer durably rejects the old ID after the cutoff. Emergency revocation disables the persona/provider mapping in durable supervisor config, stops the named peer, removes its capability mount, and recreates only the named services; no work is routed to it until a fresh capability and readiness binding exist.
...
- Provider calls have connect/first-byte/total deadlines shorter than the lease takeover bound and carry a decision-scoped cancellation token. On cancellation the supervisor commits the new epoch first, sends an authenticated `cancel` on a fresh IPC connection, and records control-commit-to-provider-ack latency; connection close alone is not evidence of cancellation. Whether cancellation is acknowledged, times out, or the independent provider service restarts, the old epoch/fence can never accept its result or publish. Recovery uses the durable provider journal and authenticated `ready`/`recover` exchanges, never a PID check or inherited request.
...
10. unauthorized control plus unauthorized peer, replay, stale capability, wrong signer/room/kind, service restart, socket replacement, and compromised-sibling IPC cases -> fail closed with zero side effects;
...
Declare four unique IPC capability secrets, three unique signer-key secrets, and one provider-credential secret. Mount each IPC capability only into `supervisor` and its peer, each signer key only into its signer, and the provider credential only into `provider`, all read-only under `/run/secrets`; no operational secret is interpolated into rendered Compose environment. `.env.example` contains paths/placeholders such as `wss://relay.invalid` and `ROOM_ID_PLACEHOLDER` only. `bootstrap_local_identity.py` creates the gitignored mode-`0400` source files before review without printing values.
...
- authenticated readiness, unauthorized/replay/stale-token, peer restart, socket replacement, and compromised-sibling tests pass before provider or live-relay use;

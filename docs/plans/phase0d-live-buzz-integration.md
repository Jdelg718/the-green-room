...
The supervisor creates exactly one `DirectorObservationSession`, performs NIP-42 with `signer-director`, and uses it only for membership discovery, room observation, and exact-ID reconciliation. The director signer signs NIP-42 `AUTH` for the configured relay and no room messages. The supervisor creates one `PersonaPublishSession` per enabled persona and authenticates it only through the matching signer. `PersonaPublishSession.submit_exact` parses persisted bytes before writing, recomputes ID, verifies signature, and rejects unless `event.pubkey == session.authenticated_pubkey == configured persona public key`, ID/kind/room are exact, and the signer/session mapping is unchanged. It also requires the current durable per-attempt transport fence/token and refuses a token after cancellation acknowledgement, deadline, lease expiry, or a higher fence. Before signing, publisher verifies `selected_persona_id -> configured public key -> authenticated ready-response public key -> PersonaPublishSession.authenticated_pubkey`; mismatch is terminal. No persona-signed bytes use the director transport.

There is one authoritative startup order. Compose first completes `ipc-init`, then starts workers so they can validate their own journal/key/credential state, create sockets, and pass only local healthchecks. The supervisor then (1) acquires the exclusive boot lock, opens the durable ledger, applies/records explicit migrations, validates path/ownership/PRAGMAs/schema/triggers/integrity and restores room/control/reconciliation state; (2) performs a fresh authenticated `ready` exchange with every configured peer and verifies public-key/policy/key-ID/journal bindings; (3) opens and authenticates the director and persona relay sessions, establishes membership/subscription readiness, and resumes any ledger-declared recovery or stop reconciliation; and only after required recovery reaches a safe blocked-or-terminal state (4) opens owner control intake and conversation admission. No peer readiness, relay intake, control mutation, reconciliation, lease recovery, or scheduling may precede successful ledger migration/integrity validation. Loss/replacement of a socket, failed authenticated readiness, policy-digest change, or peer restart makes that peer unavailable and blocks new dependent work until a new authenticated readiness exchange succeeds; in-flight ambiguity follows the journal recovery rules rather than being repeated.
...
- `rooms(room_id PRIMARY KEY, relay_namespace, paused, generation_epoch, policy_version, policy_sha256, max_selections_lifetime, selections_used_lifetime, max_model_calls_lifetime, model_calls_used_lifetime, model_calls_reserved, max_responses_outstanding, responses_outstanding, responses_published_lifetime, updated_at, FOREIGN KEY(room_id, policy_version, policy_sha256) REFERENCES policy_versions(room_id, policy_version, policy_sha256) DEFERRABLE INITIALLY DEFERRED)` with non-negative `CHECK`s, `selections_used_lifetime <= max_selections_lifetime`, `model_calls_used_lifetime + model_calls_reserved <= max_model_calls_lifetime`, and `responses_outstanding <= max_responses_outstanding`; the three `max_*` columns are constraint caches, not a second policy source, and policy triggers require them to equal the corresponding values in `canonical_policy_json` whenever the pointer changes;
- `policy_versions(room_id, policy_version, canonical_policy_json, policy_sha256, created_at, created_by_control_request_id, PRIMARY KEY(room_id, policy_version), UNIQUE(room_id, policy_version, policy_sha256), CHECK(length(policy_sha256)=64))`, an append-only table whose canonical JSON is a complete snapshot: lifetime/outstanding/model-call/content/context limits; cooldown and consecutive-turn limits; autonomous-depth/source-class rules; deterministic tie-break and eligible-persona ordering; every eligible persona ID, public key, enabled flag, configuration digest, and configuration version/ID; provider/model selection knobs that affect scheduling eligibility; retry/deadline/jitter values; and every other scheduling/admission knob. `policy_sha256` is lowercase SHA-256 of the exact canonical UTF-8 JSON bytes; timestamps and creating control request are evidence, not digest inputs;
...
- `decisions(decision_id PRIMARY KEY, room_id, source_event_id, policy_version, policy_sha256, epoch, state, persona_id NULL, reason_code, model_reservation_state TEXT NOT NULL CHECK(model_reservation_state IN ('none','reserved','consumed','released')), terminal_accounted INTEGER NOT NULL CHECK(terminal_accounted IN (0,1)), stop_reconcile_epoch INTEGER NULL, worker_owner NULL, lease_token NULL, fence INTEGER NOT NULL DEFAULT 0, transport_fence INTEGER NOT NULL DEFAULT 0, lease_until NULL, created_at, updated_at, UNIQUE(room_id, source_event_id), FOREIGN KEY(room_id, source_event_id) REFERENCES source_events(room_id, source_event_id), FOREIGN KEY(room_id, policy_version, policy_sha256) REFERENCES policy_versions(room_id, policy_version, policy_sha256))`; selected rows start `reserved/0`, non-selected terminal rows start `none/1`, consuming or releasing a reservation is monotonic, and a selected row may set `terminal_accounted` from `0` to `1` only on its one terminal-accounting transition;
...
- `publish_attempts(decision_id REFERENCES response_events(decision_id), attempt, transport_session_id, transport_boot_id, transport_token, transport_fence, state, send_started_at, send_finished_at, write_deadline_at, transport_lease_until, cancel_requested_at, cancel_ack_at, cancel_ack_result, relay_ack_state, attempted_at, PRIMARY KEY(decision_id, attempt), UNIQUE(decision_id, transport_fence), UNIQUE(transport_token))`; `state` is monotonic through `authorized -> send_started -> send_finished`, with orthogonal durable `cancel_requested`/`cancel_acknowledged` facts, and `cancel_ack_result` is exactly `no_write` or `write_possible`. Persisted times are UTC epoch milliseconds paired with `transport_boot_id`; live enforcement also uses a monotonic timer, and after restart an unverifiable prior-boot deadline is treated conservatively as write-possible until fencing/lease expiry and fresh readiness establish quiescence. There is no copied response ID: `decision_id` joins to the single authoritative `response_events.event_id`;
- `control_requests(request_id PRIMARY KEY, command, expected_epoch, resulting_epoch, result_json, result_sha256, committed_at)` for durable replay rejection; `result_json` is the complete canonical committed response (including success/rejection status, reason, epoch and aggregate counters), `result_sha256` authenticates those exact canonical bytes, and a duplicate request returns those stored bytes verbatim rather than reconstructing a response from current state;
- `room_metrics(...)` containing numeric measurements and no secret material.

`policy_version` plus `policy_sha256` is immutable evidence of the complete policy snapshot used by the **one** source decision; neither is part of source identity or a reason to reschedule. `Store.commit_policy(room_id, expected_policy_version, canonical_snapshot)` is the only write API: it validates the complete exact schema/no floats/no unknown fields, canonicalizes once, computes SHA-256, and in one `BEGIN IMMEDIATE` inserts the new append-only version and updates the room pointer and constraint caches only where its current version equals `expected_policy_version`. Zero affected room rows rolls the entire insertion back as a stale CAS; a version/digest collision with different bytes is corruption. Initial room creation inserts policy version 1 and its room pointer/caches in one transaction under the deferred foreign key; there is never a committed room without a policy or a policy without its intended room pointer. `store.py` registers deterministic `canonical_policy_valid(json)` and `sha256_hex(bytes)` SQLite functions on every writable/migration connection before schema use; insertion triggers call them, so direct SQL cannot bypass canonical completeness/digest checks. Startup's schema-integrity gate rejects missing/redefined functions or triggers. `BEFORE UPDATE` and `BEFORE DELETE` triggers always reject on `policy_versions` and `response_events`; policy insertion triggers reject non-next versions, bad digest/canonical bytes, incomplete persona/config/scheduling fields, or creator/timestamp mismatch. No general update/delete policy API exists. Triggers also reject changes to decision room/source/policy version/policy digest/epoch; reject `terminal_accounted` reversal, reservation transitions other than `reserved -> consumed|released`, and terminal states with inconsistent accounting. SQLite has no per-table runtime privilege model: least authority is enforced by exposing only narrow parameterized `Store` methods to the application, keeping raw connection/SQL objects private to `store.py`, using triggers/constraints as the database backstop, and opening evidence tooling read-only. Do not claim SQL `GRANT`-style insert/read permissions. Tests attempt forbidden updates/deletes through both the application API and direct SQL and expect rejection.
...
3. read the room's current pause flag, generation epoch, policy version/digest, lifetime counters, outstanding counters, and accepted-event sequence; join the exact immutable `policy_versions` row through the room's composite foreign key, recompute/compare its digest, and derive every limit, cooldown, eligible-persona configuration and scheduling knob only from that canonical snapshot;
...
6. insert exactly one immutable decision keyed by `(room_id, source_event_id)`, recording the current `policy_version` and `policy_sha256` as evidence only;
...
The source and decision uniqueness constraints, both independent of `policy_version`, plus `BEGIN IMMEDIATE` are the zero-or-one invariant. No network/model/signer call occurs in the transaction. A replay after a policy update returns the original decision and original recorded policy version/digest and consumes no budget. Tests race (a) two deliveries, (b) replay against an atomic policy insert/pointer update, and (c) first delivery against that policy update on separate connections; each run must yield one source, one decision, one selected persona at most, one internally consistent old-or-new reconstructable policy snapshot/digest, one room pointer, and one set of counter effects. Additional tests reopen the database and reconstruct every decision's complete limits/persona eligibility/scheduling inputs solely from its immutable policy row; reject incomplete/noncanonical/wrong-digest inserts, update/delete attempts, skipped/reused versions, stale expected-version CAS, pointer-only updates, and a decision digest/version mismatch; and prove a failed room-pointer CAS leaves no orphan policy row. A sabotage migration that restores `UNIQUE(room_id, source_event_id, policy_version)` or removes either policy foreign key/digest check must make these tests fail.
...
publishing -> reconciliation_pending -> published | publish_exhausted
...
Before first network submission, construct/sign the kind-9 response exactly once, then insert `response_events` and transition to `signed` in the same `BEGIN IMMEDIATE` transaction under the active lease/fence. With `synchronous=FULL`, success is reported only after `COMMIT` returns; no queue, callback, socket, or transport process receives `event_bytes` before that return. The publisher reopens/reads the row and verifies SHA-256, recomputed event ID, Schnorr signature, persona pubkey, room, and reply source. `Store.claim_publish(...)` then performs one final `BEGIN IMMEDIATE` epoch/token/fence CAS: it moves `signed -> publishing`, creates the first durable fenced transport attempt through the same private `claim_transport_attempt` primitive, and returns persisted bytes only with that attempt's token/fence/deadline. Later retries use that same primitive while already `publishing`; there is no byte-returning path without an attempt row. A crash at every statement boundary must prove either no handoff occurred or the exact committed response and attempt are recoverable.

Each submission is a durable, fenced transport attempt rather than an untracked socket call. In the same `BEGIN IMMEDIATE` transaction that authorizes handoff, `Store.claim_transport_attempt` increments the decision's transport fence and inserts the `publish_attempts` row with a random one-use token, session ID, `authorized` state, and an absolute write deadline no more than 5 seconds away (durable UTC/boot metadata plus a live monotonic timer). The transport must durably mark `send_started` immediately before the first WebSocket write syscall and `send_finished` immediately after that syscall resolves; both updates require the exact decision/attempt/token/fence and a live deadline/lease. It must not begin a write unless the token is current. A timeout, lost callback, cancelled task, connection close, or stale fence cannot silently imply that no write occurred.
...
Accept reconciliation only when a signature-verified returned event has the exact persisted ID, author, kind, room, content/tags, and canonical object bytes; then send `CLOSE`, mark `published`, and do not resubmit. During ordinary retry (not stop), an `EOSE` with no match permits resubmission of the **same persisted bytes** through the matching persona session only after the prior transport attempt is quiescent by the protocol below. A disconnect/timeout before a trustworthy `EOSE` is inconclusive, not proof of absence.

Transport quiescence is a prerequisite to treating any empty observation as absence. The supervisor durably records `cancel_requested`, asks the exact persona session to cancel the token and close that connection, and waits for a transport-service acknowledgement. The transport may acknowledge `no_write` only after atomically fencing the token and proving `send_started` was never recorded and no write syscall can still begin; if a write started, its completion is unknown, or the proof is unavailable, it acknowledges `write_possible`. A missing acknowledgement is never converted to `no_write`. Alternatively, after the enforced write deadline has passed, the supervisor advances the durable attempt fence, waits for the attempt's lease to expire, and verifies through authenticated session readiness that the old token cannot write; this deadline-plus-fenced-lease path establishes quiescence but not `no_write`. A late callback under the old token can update neither the attempt nor the decision and cannot initiate another write.

Only after either positive `no_write` acknowledgement or enforced-deadline-plus-fenced-lease expiry does reconciliation wait a fixed 2-second propagation grace measured from the later of send finish, cancellation acknowledgement, deadline, or connection close, then open a **fresh** director subscription using the exact query above. An `EOSE` obtained before quiescence or before that grace is diagnostic only and cannot terminalize or authorize a retry. If any attempt may have written (`send_started`, `send_finished`, `write_possible`, missing ACK, or deadline fencing), the decision remains durably `reconciliation_pending`, keeps `terminal_accounted=0`, and blocks `resume` until exact presence is verified or the bounded post-quiescence query protocol finishes. That protocol permits at most 3 fresh exact-ID subscriptions, each with a 5-second deadline and 1-second inter-query propagation delay, under one 25-second reconciliation deadline. Exact presence yields `published`; three trustworthy empty `EOSE`s after quiescence/grace yield `publish_exhausted` with reason `verified_absent_after_quiescence`; any missing trustworthy final `EOSE` at the bound yields `publish_exhausted` with an explicitly inconclusive reason. Neither outcome permits a late old-fence write, re-sign, replacement event, or unbounded query.

Publication has predeclared bounds: at most 5 submissions total, exponential backoff of 250 ms, 500 ms, 1 s, and 2 s between retries (bounded ±10% non-secret jitter), a 5-second write deadline per attempt, and a 30-second monotonic ordinary-publication deadline; a stop may consume the separate 25-second read-only reconciliation bound after transport quiescence. The epoch/fence CAS and durable attempt authorization run before every handoff. A definitive relay rejection becomes `failed`; success/verified reconciliation becomes `published`; attempt or deadline exhaustion, corrupt/missing durable bytes, verified post-quiescence absence, or inconclusive final reconciliation becomes terminal `publish_exhausted` with sanitized reason, attempt count, quiescence evidence, last transition time, and event ID available through the local status command and non-zero acceptance result. Exhaustion causes no re-signing, replacement event, unbounded background retry, or automatic budget refund. The operator may run read-only reconciliation later; only exact verified presence may move the recorded outcome to an append-only reconciliation record, never mutate signed bytes. Ten-turn acceptance fails if any selected decision is `publish_exhausted`, has unresolved acknowledgement/quiescence state, exceeds bounds, or lacks commit-before-handoff evidence.
...
No extra fields are accepted. Mutating commands require a unique 128-bit `request_id`, an `issued_at` within a predeclared 30-second wall-clock window, and `expected_epoch` equal to the current durable room epoch. `Store.apply_control` always uses `BEGIN IMMEDIATE` and returns ACK only after commit. `pause` is admission-only: it requires `paused=0`, sets `paused=1`, leaves the epoch unchanged, and lets already admitted work drain under its existing epoch. `stop` uses the stronger atomic cancellation transaction below. A duplicate committed request ID returns the exact stored canonical `result_json` bytes and digest from the original transaction without consulting current room state or performing another mutation; a new ID carrying a stale epoch is rejected. Rejections that are committed for idempotent replay store their exact canonical result too. `status` is read-only, returns no transcript/secrets, and reports pause/epoch, state counts, expired leases, counters, and any `publish_exhausted` or pending stop-reconciliation decisions. This distinction makes `pause` suitable for the planned rotation drain without stranding in-flight rows; only `stop` is the cancellation barrier.

`resume` is a fail-closed drain barrier, not a way to abandon paused work by changing its epoch. `Store.apply_resume` owns one `BEGIN IMMEDIATE` transaction. Before inserting its control result or mutating the room, it requires all of the following from the same locked snapshot:

1. the room is paused and its durable epoch is exactly `expected_epoch=E`;
2. no room decision at epoch `E` has `terminal_accounted=0`, and no epoch-`E` decision is in `selected`, `generating`, `signed`, `publishing`, or any state a migration declares nonterminal or reconciliation-pending;
3. across every epoch, there is no `publishing` decision, non-null `stop_reconcile_epoch`, live stop-reconciliation lease, or durable publish/reconciliation attempt whose latest outcome is pending, ambiguous, or inconclusive; terminal outcomes with counters already accounted are not reclassified by resume;
4. `rooms.responses_outstanding` equals the exact count of that room's decisions with `terminal_accounted=0`, `rooms.model_calls_reserved` equals the exact count with `model_reservation_state='reserved'`, every reserved row is nonterminal and unaccounted, every terminal-accounted row is not reserved, all counters are non-negative, and the lifetime/reservation caps and schema invariants still hold.

If any query, aggregate, state allowlist, or invariant fails, the transaction rolls back: `paused`, epoch, counters, reservations, leases, and `control_requests` remain byte-for-byte unchanged, and no signal or worker/network action follows. Only after every check succeeds does that same transaction insert the replay-protected successful control result, set `paused=0`, and advance the room once from `E` to `E+1`. This serialization keeps ordinary current-epoch/token/fence CAS coherent: a worker transition that commits first makes resume observe outstanding work and reject; a successful resume proves the paused epoch was drained, after which new admissions use only `E+1`. Resume has no old-epoch writer privilege and does not weaken the stop-only path.

The supervisor signals tasks only after the applicable database transaction commits; the durable room state and fences—not the signal—are authoritative. Startup follows the sole order in §4.3: ledger lock/migrations/integrity/state restoration first, authenticated peer and relay readiness second, then owner/control and conversation intake. It completes any pending stop reconciliation before allowing `resume` and never resets epoch to a default. Socket replacement and restart race tests prove there is one listener and one committed epoch history. Duplicate-control tests mutate room state after the original ACK, restart, then prove replay returns byte-for-byte identical stored result JSON/digest rather than a reconstruction. Unauthorized tests cover wrong peer UID, mode `0666`, symlink/socket substitution, malformed/oversized/unknown commands, stale timestamp, stale expected epoch, duplicate request ID before and after restart, concurrent pause/resume/stop, and attempts from provider/signer services; all must produce no unauthorized state change and no network/model/signer activity.
...
3. bump-fences **every** old-epoch nonterminal decision (`epoch=E` and state in `selected,generating,signed,publishing`) by incrementing worker `fence` and clearing `worker_owner`, `lease_token`, and `lease_until`; for `publishing`, also increment `transport_fence`, so every stale worker and every not-yet-started transport token fails its ordinary CAS;
...
Post-handoff reconciliation is deliberately separate from cancellation. A `publishing` row means `claim_publish` won before the stop transaction and a socket handoff may have occurred; stop cannot truthfully recall or cancel it. Immediately after stop commits, the supervisor requests cancellation and connection close for every current transport token and durably records each request/ACK. While the room remains paused, only `Store.claim_stop_reconciliation(decision_id, stop_reconcile_epoch)` may issue a fresh reconciliation lease. It requires state `publishing` or `reconciliation_pending`, the recorded stop epoch equal to the room's current epoch, `terminal_accounted=0`, and no live reconciliation lease; it moves `publishing` to `reconciliation_pending` but never returns bytes to a persona publication session. The holder first proves transport quiescence and then performs only the propagation grace and bounded fresh exact-ID reads defined in §5.4. An early empty `EOSE`, including one received while an already-authorized write is delayed, has no state/counter effect. Exact verified presence transitions to `published`; only a post-quiescence/grace definitive query bound may transition to `publish_exhausted`. That terminal transaction compares the recorded stop epoch plus reconciliation token/fence, sets `terminal_accounted=1`, decrements `responses_outstanding` once, increments `responses_published_lifetime` only for exact presence, and clears `stop_reconcile_epoch`. It never resubmits, re-signs, replaces bytes, or refunds lifetime/provider usage. `resume` is rejected until no such row remains.

The `BEGIN IMMEDIATE` ordering makes the race explicit: if `stop` acquires the write transaction before `claim_publish`, the old-epoch `signed` row is cancelled and no bytes are returned; if `claim_publish` commits first, the row is `publishing` and goes through quiescence plus read-only post-handoff reconciliation, never false cancellation or premature absence. A transport write racing after a committed claim remains the documented database/socket boundary and is reported as pre-stop authorization with potentially post-stop observation, not as a post-stop scheduling success.
...
RED uses two SQLite connections racing on the same source event and initially observes the missing schema/claim behavior. Add the policy-independent source/decision uniqueness, complete append-only `policy_versions` snapshots/digests, composite policy foreign keys, atomic expected-version policy CAS, durable canonical control results, counter constraints, and `claim_and_schedule`. GREEN must show one decision row, at most one selected persona, one set of counter effects, an identical duplicate decision, and byte-identical duplicate control responses. Add replay-after-policy-change and first-delivery/policy-update races; full decision-policy reconstruction after reopen; stale update CAS/orphan rollback; malformed/incomplete/wrong-digest policy rejection; direct/API policy update/delete rejection; policy pointer/version/digest FK violations; then sabotage decision uniqueness and policy FK/digest enforcement and prove those tests fail. Add crash/reopen, namespace, pause, cooldown, lifetime-versus-outstanding accounting, every cancellation/failure counter effect, SQLite trigger/API restriction, non-tmpfs path validation, required-PRAGMA readback, and rollback-on-exception tests.
...
RED blocks mock provider work and, separately, signer work after an admission-only `pause`, then attempts a premature `resume`; the naive epoch bump strands the `selected`/`generating`/`signed` row and leaks outstanding or reserved accounting because ordinary CAS can no longer match. GREEN adds the mode-`0600` owner Unix control socket, durable request/epoch CAS, the fail-closed atomic resume drain barrier, the atomic stop-only old-epoch bulk transition/counter transaction, worker token/fence CAS, authenticated provider cancellation, durable supervisor lock, and service-restart recovery through `ready`/`recover`. Test all unauthorized/replay/stale controls from §6.1; simultaneous lease claims; heartbeat/takeover; strict expired-lease takeover only after authenticated journal recovery; and an old worker result returning after the stop fence.

Resume tests use separate SQLite connections and deterministic latches. Hold provider work in `generating` and signer work before/after the central `signed` commit, issue `pause`, and prove each premature resume is rejected with no control row and no change to pause, epoch, decision state, fences, counters, or reservations. Restart the supervisor against the same database while work is still blocked and prove resume rejects identically. Release the latch, let the exact original-epoch work terminalize and all counters/reservations and publish/stop reconciliation reconcile, then prove one fresh resume commits `E -> E+1` exactly once. Replaying that committed request ID returns its stored ACK without another increment, while a fresh request carrying stale `expected_epoch=E` is rejected with no mutation. Race the final worker terminal transaction against resume: worker-first permits the one successful resume only after the aggregate checks see the terminal commit; resume-first must reject rather than strand work. Repeat for `selected`, `generating`, `signed`, `publishing`, stop-reconciliation markers/leases, pending or ambiguous publish outcomes, a future synthetic reconciliation-pending state, and deliberately corrupted outstanding/reservation aggregates; every failed check rolls back the request insertion and all room fields.

Stop barrier tests use separate SQLite connections and deterministic latches to race stop against (a) selected admission, (b) reservation consumption/provider generation, (c) generated output before signer request, (d) signer completion before central `signed` commit, (e) committed `signed` before `claim_publish`, and (f) `claim_publish`/socket handoff. For (a)-(e), stop-first yields one old-epoch `cancelled` row, no signer/publish after the barrier, exact `terminal_accounted`, reservation, used-call, and outstanding counters, and every stale ordinary CAS affects zero rows; worker-first may advance only to the next declared pre-submit state before the same stop bulk-cancels it. For (f), stop-first cancels `signed` with zero transport writes, while claim-first leaves `publishing`, requests token cancellation/connection close, bump-fences the old publisher, performs no resubmission, and resolves only after durable transport quiescence plus bounded fresh exact-ID observation to `published` or `publish_exhausted`, never false `cancelled`. Add the deterministic delayed-write regression: latch an authorized attempt after durable `send_started` but before relay visibility/completion, issue stop, deliver an early trustworthy empty `EOSE`, and prove the row remains `reconciliation_pending` and unaccounted; then release the already-authorized write, expose the exact event after the propagation delay, and prove a fresh reconciliation yields `published` with one outstanding decrement/publication increment. Repeat with stop replay, concurrent stop requests, missing/`write_possible` cancellation ACK, enforced deadline plus fenced-lease expiry, trigger/error rollback, and restart during pending reconciliation; assert epoch/control/counter mutations occur once and resume cannot resurrect old work. Also test provider and supervisor restart and exactly one provider network operation despite fresh authenticated IPC retries.

### Slice 7 — sign-once publisher

RED simulates accepted publish with lost `OK`, crash/restart at each signer-journal/central-commit/handoff boundary, reconnect, and retry; a naive regenerated event, pre-commit handoff, unfenced delayed write, or early empty-EOSE terminalization would fail. GREEN adds isolated per-identity signer services, per-peer socket volumes and generation-named HMAC capabilities, the exact peer-specific supervisor/local-peer mount targets, durable IPC replay cache and request-hash idempotency, signer-side relay/challenge/pubkey/kind/room enforcement, `synchronous=FULL` commit-before-handoff, per-attempt send-start/send-finish/deadline/cancel-request/cancel-ACK state and transport tokens/fences, exact reconciliation query, propagation grace, and bounded retry/exhaustion. Test unauthorized peer/capability and bad MAC; duplicate request ID/nonce and replay across restart; stale/unknown/revoked key ID; wrong signer/persona/room/relay/kind; stale/replayed NIP-42 challenge; same-ID/different-hash reuse; interrupted signer intent; journaled-result recovery; socket unlink/replacement/symlink/wrong-owner/wrong-mode; response substitution; compromised sibling with and without a deliberately exposed path; existing response row; stale lease/fence/transport token; byte corruption; signer/session/persona mismatch; director transport lacking publish; exact match before retry; pre-quiescence empty `EOSE`; `no_write` and `write_possible` cancellation ACKs; missing ACK with deadline-plus-fence quiescence; inconclusive timeout; definitive rejection; duplicate/lost `OK`; fixed retry/deadline bounds; operator-visible `publish_exhausted`; non-zero acceptance on exhaustion; and no custom provenance tag. Add a static-secret rotation test that proves running containers do not see staged files, requires coordinated old+next force-recreate, authenticates staged-next readiness, rejects normal staged-next work before activation, atomically activates/revokes in peer journals plus the supervisor binding transaction, fails old authentication after cutover/restart, and requires a second force-recreate to remove old mounts; exercise rollback both before activation and forward-only recovery after revocation. Assert one cryptographic signing operation and byte-for-byte identical journal/central/submission bytes; repeated IPC uses fresh envelopes and may only retrieve the journaled result.
...
Required mock scenarios (exactly 17 acceptance scenario groups; each group may contain focused parameterized cases):

1. one human event -> one selected persona -> one published event;
2. deliberate silence -> zero model calls/publishes;
3. duplicate live/replay plus two contending workers -> one decision/model call/signer call/publish ID;
4. atomic immutable-policy update races first delivery -> one internally consistent old-or-new version/digest and no orphan row;
5. replay after policy update -> original decision and fully reconstructable original canonical policy, with no new budget effect;
6. malformed/incomplete/noncanonical/wrong-digest policy, stale expected-version CAS, direct update/delete, and decision/pointer FK mismatch -> fail closed with no mutation;
7. persona event -> zero schedules;
8. cooldown and budget exhaustion -> durable silence;
9. cancellation during generation -> no publish;
10. lost ACK/reconnect -> exact query reconciliation and exact same committed-byte retry within bounds;
11. delayed authorized socket write + stop + early empty `EOSE` -> remains `reconciliation_pending`; late exact write then fresh post-grace query -> `published`, never false absence;
12. transport cancellation `no_write`, `write_possible`, missing ACK, deadline/fenced-lease expiry, stale token, and query-bound cases -> only post-quiescence/grace terminal outcomes and no late write;
13. malformed/unauthorized event -> adapter rejection and zero model calls;
14. restart -> dedup, immutable policy evidence, byte-identical canonical control replay result, epoch, transport/worker leases/fences, budgets, and cooldown persist;
15. unauthorized control plus unauthorized peer, replay, stale/revoked capability, wrong signer/room/kind, service restart, socket replacement, compromised-sibling IPC, and relay control-shaped conversation cases -> fail closed with zero control-plane or worker side effects;
16. publish exhaustion -> durable operator-visible terminal state and acceptance failure;
17. blocked provider/signing work plus admission-only pause -> premature resume fails closed before and after restart with no epoch/counter/control mutation; after terminal accounting and reconciliation drain, resume commits exactly once, duplicate request returns byte-identical original result, and stale-epoch fresh request fails.
...
- duplicate replay under a changed policy returns the original decision/version/digest and reconstructs the complete original canonical limits/persona configuration/scheduling inputs; atomic expected-version policy-update races choose one complete old-or-new snapshot and leave no orphan version; persona-authored events, cooldown/limit silence, and cancellation produce no fan-out;
...
- every signed event was durably committed before first handoff, all submissions for it are byte-identical, each attempt has complete send/deadline/cancel/fence evidence, and lease contention/stale-worker return causes no duplicate provider network request, cryptographic signing operation, or late old-fence write (journal-result retrieval is counted separately);
- no `publish_exhausted`, unresolved acknowledgement/quiescence, pre-grace absence decision, stale lease/fence write, leaked reservation, or retry/deadline violation exists;
- every successful resume is backed by one atomic paused/drained/counter-consistent snapshot; deterministic premature-resume and restart cases leave epoch, controls, counters, and reservations unchanged, and replay cannot increment the epoch twice;
...
- **safety counters:** adapter rejects, duplicate claims, policy-version/digest replay returns, policy CAS conflicts, lease conflicts/takeovers/stale-fence rejects, provider/signer IPC requests per decision, stale epochs, transport send starts/finishes/deadlines/cancel requests/ACK classes/fence rejects, pre-quiescence empty EOSEs ignored, publish/reconciliation attempts, relay rejects, exhausted publications, unauthorized control/IPC rejects, late results discarded, max simultaneous provider calls, and maximum responses per source event.
...
2. **Quality review:** inspect policy-independent dedup, complete append-only policy snapshots/digests and atomic pointer CAS, reconstructability and policy race handling, transaction/counter boundaries including `terminal_accounted`/reservation state, fenced worker and transport leases/takeover, the fail-closed atomic resume drain/counter contract, atomic stop-only old-epoch bulk cancellation, transport cancellation/quiescence before separate post-handoff reconciliation, state machine, commit-before-handoff, propagation grace/exact reconciliation/bounded retries, canonical committed control-result replay, crash behavior, test determinism, typed conversation-only event interfaces, dependency necessity, structured logs, and absence of hidden network calls. Sabotage dedup uniqueness, policy immutability/FK/digest enforcement, lease/transport fencing, one resume drain/aggregate predicate, stop bulk predicate/counter accounting, quiescence-before-empty-EOSE, commit-before-handoff, and ordinary epoch checks to prove their tests fail, then restore and rerun.
...
2. Confirm every old-epoch `selected`/`generating`/`signed` decision is durably cancelled with counters reconciled, every provider cancellation is acknowledged or marked ambiguous for journal recovery, every stale lease/fence is rejected, and no new old-epoch `publishing` transition occurs. For each already-authorized transport attempt, durably request cancellation/connection close and obtain `no_write` acknowledgement or enforce deadline plus fenced-lease expiry; only then wait propagation grace and run fresh bounded exact-ID queries. Keep write-possible attempts `reconciliation_pending` until that protocol finds exact presence or reaches its declared query bound, and surface exhaustion rather than retry forever.
...
- If acknowledgement ambiguity remains, first establish transport quiescence, then use the fresh post-grace exact bounded reconciliation queries and record `publish_exhausted`; never use an early empty `EOSE`, regenerate a response, or silently claim rollback succeeded.
...
- mock tests pass before live tests and include the 17 required scenario groups: complete immutable-policy update/replay/reconstruction/races, lease fencing and expired-worker return, separate-service authenticated IPC/signer isolation, unauthorized peer/replay/stale-token/socket-replacement/compromised-sibling cases, byte-identical replayed controls, cancellation epoch, lifetime/outstanding counter persistence, durable sign-once transport quiescence/delayed-write/reconciliation/exhaustion, and no persona recursion;
...
- the durable SQLite ledger mechanically proves one policy-independent decision and zero-or-one selected/published response for every source event, a reconstructable immutable policy version/digest, one authoritative response event ID, exact counter reconciliation, and commit-before-fenced-handoff evidence;
- setup time, idle resources, latency, model calls, and safety counters are captured rather than estimated;
- rollback, membership removal, owner-authenticated immediate stop, fail-closed pause/drain/resume before and after restart, restart/epoch preservation, and operator-visible exhausted-state behavior are exercised;

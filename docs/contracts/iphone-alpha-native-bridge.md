...
The TypeScript core may build the bounded role/content messages and interpret only the sanitized result. A successful generation also returns the positive integer `attemptEpoch` owned by that exact native provider attempt; completion binds it back to SQLite so an older callback cannot complete a newer retry. No project service receives those messages. This foundation implements the schema-8 native consent authority, canonical provider disclosure data, and fail-closed enforcement only. The final unchecked consent UI, offline Privacy & Data Use screen, selection-reset interaction, credential-removal UI, and confirmation/recovery UX remain unimplemented follow-up work under issue #208; this contract does not claim external-release readiness or approved public-policy wording.

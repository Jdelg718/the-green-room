...
The TypeScript core may build the bounded role/content messages and interpret only the sanitized result. A successful generation also returns the positive integer `attemptEpoch` owned by that exact native provider attempt; completion binds it back to SQLite so an older callback cannot complete a newer retry. No project service receives those messages. The schema-8 native authority is consumed by the bundled provider UI: canonical generated disclosure data names the selected fixed provider/host, consent starts unchecked and resets on every provider/model edit, recorded state appears only after native readback, and valid saves use `database.saveProviderSelectionAndConsent` before credential entry when needed. The offline Privacy & Data Use screen is bound before database open. Credential removal UI, abandon confirmation, and broader recovery-copy work remain follow-up Slice B under issue #208.

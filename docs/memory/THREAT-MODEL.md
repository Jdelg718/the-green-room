...
This threat model covers the authoritative local SQLite store, harness-to-adapter projection boundary, managed Obsidian subtree, HTTP sink, export/import, and derived-memory use in prompts. Protected assets are committed room order, provider-context snapshots, conversation events, derived memories, room/persona/human identity partitioning, availability, provenance/lineage, adapter credentials, consent/deletion state, and the integrity of files and network destinations.
...
| Split authority / false acknowledgement | Optional sink allocates order, is ahead, or reports success while SQLite differs. | SQLite commits and orders first; sink only echoes IDs/positions; gaps/conflicts stop projection; room acknowledgements never depend on sink. |
| Cross-room/identity confusion | Sink result from another room/persona/member enters provider context. | Adapter-instance + room partition; directional scope; immutable actor provenance; revalidate every cited event and visibility against authority snapshot. |
...
| Sink outage / stale context | Unavailable sink loses a correction or serves forgotten content. | Bounded idempotent outbox, visible lag/degraded state, no unapproved fallback; local tombstone immediately excludes provider context before propagation. |
...
| Consent/data-location surprise | Invited-human content is projected to a sink not disclosed at join. | Explicit local enablement plus participant-visible sink/location/retention/deletion disclosure; material changes require re-consent and context exclusion. |
...
HTTP contract 1.0 is a projection/query profile only and advertises `export_import: false`; export/import return `unsupported`. The 64 MiB deterministic bundle is transferred only inside the local companion through the retained, digest-and-length-bound harness byte-stream contract. No response URL, callback, ambient file path, or second network fetch is permitted.
...
## Platform, permissions, encryption, and packaging gates

The schemas do not prove a shippable adapter. Writable release support requires measured no-follow/locking/durability behavior, user-only directory and file permissions, secret-store integration, authority/outbox recovery, backup/restore, and clean removal on each supported OS/filesystem. Windows reparse/ACL behavior, macOS and Linux permission behavior, and network/cloud filesystems are separate gates. Node 24 is the current runtime baseline. Docker, desktop wrappers, public sync, clean-host installers, filesystem encryption, and E2EE are not implied by this contract; documentation and UI must state which OS/user/sync/provider can read plaintext. Packs remain declarative data and cannot ship adapter code or configuration.

...
- Reviewed protected-main baseline: commit `03d71d4cb803b4b495101fd582bfd7248ba10856`, tree `6eb14984dcaae00c9c1b857331203f141428258f`.
...
- [ ] Archive and IPA signing must both be Apple Distribution for Team `JZ233HBW3Z` and exact profile `Green Room App Store Connect 0.1.0 Build 1`. Signed app entitlements remain exactly the five policy values, including the exact app-specific keychain group, `get-task-allow=false`, and `beta-reports-active=true`. The provisioning profile is validated separately as an App Store authorization superset: exact-team wildcard or exact application identifier, exact team, distribution booleans, no device/enterprise traits, and only the exact app group or team wildcard plus Apple's `com.apple.token` keychain group. A profile must still authorize the exact signed app group; wrong-team wildcards, unrelated groups, token-only authorization, extra entitlements, or signed-app widening fail closed.
...
- [ ] Re-audit the exact descriptor-bound export snapshot independently: require its returned inventory hash to equal both pre- and post-audit live inventories, bind the snapshot IPA SHA-256 and byte size, export-evidence SHA-256, and a separately labeled immutable extracted-payload snapshot inventory; verify identity/source declaration, exact signed app entitlements, the separately bounded App Store profile authorization, privacy, schema, assets, fixed endpoints, and prohibited payloads in every recursively detected Mach-O. The IPA parser accepts only stored/deflated ZIP32 entries with optional UTF-8 and standard bit-3 data descriptors, treats central-directory CRC/sizes as authoritative, and requires each signed or unsigned descriptor to match exactly and end at the next local header or central directory. Encryption, ZIP64, multidisk, unsupported flags/compression, malformed or ambiguous descriptors, path traversal, duplicate paths, symlinks/special files, bombs, overlaps, gaps, and trailing/smuggled bytes remain rejected. Reject every Mach-O except the exact app, Capacitor, and Cordova binaries, including FAT64 `cafebabf`/`bfbafeca` files.

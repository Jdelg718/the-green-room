...
## Standalone iPhone and future collaboration boundary

[ADR 0006](adr/0006-standalone-iphone-capacitor-runtime.md) accepts a separate, fully standalone iPhone Alpha for issue #160. An iPhone-local room has no Mac companion: a bundled Capacitor/`WKWebView` presentation and pure TypeScript core run the room and bounded director in process; native Swift bridges own SQLite, Keychain, and fixed-definition `URLSession` cloud-provider calls. SQLite is authoritative for iPhone-local rooms. The signed bundle contains all application JavaScript and all 19 personas, downloads no executable code, exposes no local server, and does not synchronize with desktop rooms. The [implementation plan](plans/2026-09-05-standalone-iphone-alpha.md) and [native bridge contract](contracts/iphone-alpha-native-bridge.md) fix the Alpha phases and security boundary.

The desktop local companion remains authoritative for desktop-created rooms. Cross-device migration, synchronization, pairing, human invitations, accounts, or relay work remains future architecture and requires a separate accepted ADR; the prior shared Node/Swift contract fixtures remain useful evidence for that later track but are not the standalone app's transport. iPad specialization is also deferred.

On iPhone, provider key bytes are accepted only by a native setup surface/bridge, stored as non-synchronizing `WhenUnlockedThisDeviceOnly` Keychain items, resolved only inside the native provider bridge, and never returned to JavaScript or written to preferences, SQLite, WebKit stores, logs, events, diagnostics, exports, backups, or project-operated services. The UI must disclose that a selected provider receives bounded persona and room context. Offline existing-room access is explicitly read-only; drafts are `Not sent`, and no operation appears acknowledged before its SQLite transaction commits.

The desktop [local room library](ROOM-LIBRARY.md) keeps stable opaque room IDs and room-scoped API paths while SQLite ordered events remain authoritative. Its durable selected-room pointer is navigation state only; it never replaces per-room event order, idempotency, cast, director, provider-work, or replay authority.

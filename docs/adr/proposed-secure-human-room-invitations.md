...
2. The first collaboration slice is direct and private: same LAN or private Tailscale reachability with authenticated TLS. If authenticated TLS cannot be established, invitation inspection, consent, redemption, membership, reconnect, and room traffic fail closed. Plaintext HTTP is permitted only for same-device loopback development and is never an invited-human transport. The slice has no Green Room account, relay, push service, hosted inference, or provider-key movement.

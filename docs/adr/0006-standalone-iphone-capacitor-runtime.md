...
- **Persistence:** derive a separate iPhone migration series from the current schema and event/command invariants; do not reuse desktop seed data or claim byte-identical migrations. The repository-owned native bridge opens exactly one app-owned SQLite database, verifies required SQLite runtime features, enables foreign keys and WAL, applies checksum-verified consecutive migrations, exposes parameterized transaction/query operations, and places the database and WAL/SHM files under complete file protection. Schema 7 adds one immutable generation command per unresolved room turn. Preparing that command stores the proposed human event, director decision/state, and provider request digest without changing events, director state, room sequence, or activity. Only command completion atomically exposes the human/director/persona triplet (or human/director deliberate-silence pair) and deletes the matching draft. No room content goes to `localStorage`, IndexedDB, Capacitor Preferences, Keychain, logs, diagnostics, or iCloud documents; explicitly local unsent drafts live only in a dedicated SQLite table and never become acknowledged events or an automatic outbox.
...
- **Providers:** Alpha supports only the current approved OpenAI-compatible cloud definitions (`openrouter`, `openai`, `xai`, `groq`, and `together`) after per-provider contract tests pass. JavaScript passes only the exact command ID, request ID, and SHA-256 request digest. Swift reloads the immutable plan, verifies its digest and current credential/profile/generation fences, changes the command to `in_flight`, retains the suspended task before starting it, and returns text only while the same attempt and lifecycle epoch remain current. Swift owns fixed HTTPS scheme/host/port/path construction, authorization headers, redirect denial, body/response/time/concurrency bounds, cancellation, and sanitized failures. No arbitrary endpoint, header, URL, HTTP exception, web-view `fetch`, or provider SDK is permitted.
- **Lifecycle:** foreground operations are task-owned and cancellable. Scene resignation/background and protected-data loss fence the lifecycle epoch, cancel retained provider work, mark started work `interrupted`, checkpoint, and close SQLite. Activation and cold launch reopen/migrate/reconcile the database, change stranded `in_flight` work to `interrupted`, change provably unstarted `prepared` work to precise `failed/not_started`, and derive the room projection only from committed events before enabling mutations. Relaunch never performs a provider request. Existing rooms open through queries only; choosing a room for viewing does not rewrite `current_room`, including offline. There is no background execution entitlement, push, hidden outbox, automatic retry, or false acknowledgement.

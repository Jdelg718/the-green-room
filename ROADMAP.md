...
**Current checkpoint (2026-09-13):** the standalone iPhone Alpha is verified through internal TestFlight. Build `0.1.0 (2)` from protected-main commit `2918846bb7b652d2b01626ab8587c134dd4bd2e0` is processed as TestFlight Internal Only, assigned only to the one-member `Internal Alpha` group, installed on Kent's physical iPhone, and displays the merged `GR//` app icon. Update-install readback preserved seven rooms, 78 durable events, and a `ready` Keychain credential lifecycle; a 30-file app-container scan found no OpenRouter-key-shaped value. The schema-7 atomic-turn lifecycle matrix passed real provider replies, offline fail-closed behavior, protected-data recovery, forced in-flight interruption, explicit exact-command retry, and at-most-one committed event triplet. External TestFlight, public links, website publication, Beta App Review, and App Store submission remain separate human gates. Linux source mode still needs its real Omarchy cloud-provider run; Windows remains unsupported.

...
**Current checkpoint (2026-09-18):** protected-main base `14f48401cfac12f740f369ad0e658382473fb171` is prepared forward as external-candidate identity `0.1.0 (5)` with an explicit offline Review Demo for Apple Guideline 2.1(a). Active policy permits local archive/export/audit only; upload, public links, tester changes, App Store submission, and production release remain forbidden. The candidate remains pending protected review/CI, exact protected-main readback, fresh source-manifest review, and audited local archive/export. Physical and Apple-side acceptance remain separate human gates.
...
1. Merge and read back the build-5 offline Review Demo source freeze, then regenerate and independently review the exact no-upload source manifest against that clean protected-main SHA.
2. Produce and audit the exact build-5 Apple Distribution archive/export locally. Stop before upload; App Store Connect mutation, Beta App Review, tester changes, invitations, and public links remain separate human gates.
3. After separate authorization, complete physical acceptance for build 5: exact identity/source readback, Review Demo opt-in and offline persistence, retained provider rooms/events, consent-required migration, Keychain continuity/removal/re-entry, one direct fixed-provider turn, offline existing-room behavior, lock/unlock recovery, force-quit/exact-command retry, secret-free bounded evidence, and the manual VoiceOver/Switch Control/Voice Control/Dynamic Type/contrast/motion/keyboard matrix.

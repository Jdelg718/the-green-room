...
**Current checkpoint (2026-09-14):** installed product source `1bc44de336ad7be58f43cdb55556af129cc20605`—protected `main` at build/install time—freezes external-candidate identity `0.1.0 (3)`, publishes the reviewed privacy policy, and contains the accessibility, schema-8 provider-consent authority, credential-removal, confirmation, and recovery UX required for limited external TestFlight evaluation. A clean Apple Development-signed Release build from that exact SHA passed the Release bundle verifier and was installed over internal TestFlight build `0.1.0 (2)` on Kent's iPhone 15 Pro Max without uninstalling. Device readback confirmed build 3; retained-room/Keychain behavior and the manual accessibility/lock/offline/provider matrix were **not run before the session stopped**, so build 3 is installed but not physically accepted. Documentation-only handoff commits may advance `main`; do not relabel the installed source or reuse a stale manifest. No archive, distribution signing, IPA export/upload, Beta App Review, external group/link, or App Store submission occurred. Linux source mode still needs its real Omarchy cloud-provider run; Windows remains unsupported.
...
1. Reconnect Kent's iPhone and complete build-3 physical acceptance from exact protected main: retained rooms/events, consent-required migration, Keychain continuity/removal/re-entry, one direct fixed-provider turn, offline existing-room behavior, lock/unlock recovery, force-quit/exact-command retry, secret-free bounded evidence, and the manual VoiceOver/Switch Control/Voice Control/Dynamic Type/contrast/motion/keyboard matrix.
2. If physical acceptance passes, regenerate and review the exact no-upload source manifest against the then-current protected-main SHA. Stop for Kent's separate approval before any Apple Distribution archive, IPA export/upload, or Beta App Review submission.
3. After Apple approval, stop again for approval of the external tester group/cap, invitation/public-link mode, and any `greenroomai.net` placement. This remains TestFlight distribution, not a direct download or App Store release.

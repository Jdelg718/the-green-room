...
**Current checkpoint (2026-09-08):** the standalone iPhone first milestone is proven on Kent's physical iPhone, including 19 bundled portraits/prompts, Keychain-only OpenRouter setup, real character replies, explicit or automatic speaker selection, and force-quit/relaunch persistence. Issue #192's schema-7 atomic-turn/lifecycle correction is complete on rebased local commit `86831d20e2885a183988360f829de1a676996abb`: focused contracts, native Swift harnesses, Debug and Release Simulator builds, bundle verification, offline launch, and the full Node check pass. It remains unpushed and requires the physical-iPhone interruption, protected-data, exact-retry, and at-most-one-triplet matrix after Xcode authorization; see the [issue #192 handoff](docs/handoffs/2026-09-08-issue-192-iphone-atomic-turn-lifecycle.md). Linux source mode has an opt-in `0600` file credential store, `npm run start:linux`, a repository Node 24.20.0 pin, and exact-byte checkouts on every platform through `.gitattributes`; the deterministic launcher smoke, full Node check, Python suite, and first-playable acceptance pass on Kent's Omarchy desktop, but the human run with a locally entered real cloud key remains outstanding. Windows was inventoried on real hardware in the [2026-09-08 execution plan](docs/plans/2026-09-08-next-day-execution.md): strict install, build, and loopback server start work, while the launchers, the source-default inspection mode, and the file credential store fail closed; Windows remains unsupported. The next iPhone milestone is a deliberately limited TestFlight alpha, not App Store release.
...
2. Push and review the rebased issue #192 atomic-turn/lifecycle commit, then complete Xcode authorization and the handoff's physical-iPhone interruption, protected-data, exact-retry, and at-most-one-triplet matrix before selecting the TestFlight candidate commit.
...
**Atomic lifecycle correction ready for device qualification (2026-09-08):** issue #192 is implemented on rebased local commit `86831d20e2885a183988360f829de1a676996abb`. Schema 7 now prepares an immutable durable command without exposing events, commits a successful human/director/persona turn in one SQLite boundary, preserves Not sent drafts across definitive/uncertain failure, requires explicit exact-command retry, and fences native provider work across path, lifecycle, protected-data, and late-callback races. Automated source, native, Simulator, release-boundary, offline, bundle, and full-check gates pass. The branch is not yet pushed or merged, and the physical-iPhone matrix remains a real acceptance gate rather than inferred evidence; follow the [issue #192 handoff](docs/handoffs/2026-09-08-issue-192-iphone-atomic-turn-lifecycle.md).

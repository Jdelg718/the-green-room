    ...
    for line in lines:
        if line.startswith("  - "):
            ...
            assert value.startswith('"') and value.endswith('"'), (
                f"{path}: list strings must be quoted"
            )
...
def validate_record_semantics(record: dict, events: dict[str, dict]) -> None:
    ...
    if (
        ...
        and scope["source_persona_id"] == scope["target_persona_id"]
    ):
...
def validate_export_semantics(request: dict) -> None:
    room_ids = request["room_ids"]
    classifications = request["classifications"]
    snapshot_room_ids = [snapshot["room_id"] for snapshot in request["snapshots"]]
    if room_ids != sorted(set(room_ids)):
        raise ValueError("room_ids must be sorted and unique")
    if classifications != sorted(set(classifications)):
        raise ValueError("classifications must be sorted and unique")
    if snapshot_room_ids != room_ids:
        raise ValueError("snapshots must contain exactly one entry per selected room in order")


def validate_manifest_semantics(manifest: dict) -> None:
    rooms = manifest["rooms"]
    classifications = manifest["classifications"]
    snapshot_room_ids = [snapshot["room_id"] for snapshot in manifest["snapshots"]]
    if rooms != sorted(set(rooms)):
        raise ValueError("manifest rooms must be sorted and unique")
    if classifications != sorted(set(classifications)):
        raise ValueError("manifest classifications must be sorted and unique")
    if snapshot_room_ids != rooms:
        raise ValueError("manifest snapshots must contain exactly one entry per room in order")
...
def test_schemas_and_fixtures() -> None:
    ...
    expected.update(
        f"{operation}-{direction}.schema.json"
    ...
    assert not Draft202012Validator(schemas["health-response.schema.json"]).is_valid(
        health_bad_error
    )
    ...
    valid = sorted(
        path
        for path in (ADAPTER_FIXTURES / "valid").glob("*.json")
        if path.name != "operation-envelopes.json"
    )
    assert len(valid) == 6
    for path in valid:
        data = load_json(path)
    ...
    assert set(failures) == {path.name for path in invalid}, (
        f"invalid fixtures unexpectedly passed: {failures}"
    )
    ...
    assert not record_schema.is_valid(missing_lineage), (
        "compaction without lineage unexpectedly passed"
    )
    ...
    assert not retrieve_schema.is_valid(half_direction), (
        "half-directional retrieval scope unexpectedly passed"
    )
    ...
    manifest = {
        ...
        "snapshots": [
            {
                ...
                "policy_snapshot_digest": "sha256:" + "a" * 64,
                "provider_context_snapshot_digest": "sha256:" + "b" * 64,
            }
        ],
        "counts": {
            "rooms": 1,
        ...
        "entries": [
            {
                "path": f"rooms/{event['room_id']}/events.ndjson",
                "bytes": 432,
                "sha256": "sha256:" + "d" * 64,
                "items": 1,
            }
    ...
    for required in (
        ...
        assert not export_manifest.is_valid(incomplete), (
            f"manifest without {required} unexpectedly passed"
        )

    export_item = next(item for item in envelopes if item["operation"] == "export")
    validate_export_semantics(export_item["request"])
    for mutation in ("duplicate_room", "duplicate_snapshot", "missing_snapshot"):
        invalid_export = json.loads(json.dumps(export_item["request"]))
        if mutation == "duplicate_room":
            invalid_export["room_ids"].append(invalid_export["room_ids"][0])
        elif mutation == "duplicate_snapshot":
            invalid_export["snapshots"].append(invalid_export["snapshots"][0])
        else:
            invalid_export["snapshots"] = []
        try:
            Draft202012Validator(schemas["export-request.schema.json"]).validate(invalid_export)
            validate_export_semantics(invalid_export)
        except (ValidationError, ValueError):
            continue
        raise AssertionError(f"{mutation} export selection unexpectedly passed")

    for mutation in ("duplicate_room", "duplicate_snapshot", "missing_snapshot"):
        invalid_manifest = json.loads(json.dumps(manifest))
        if mutation == "duplicate_room":
            invalid_manifest["rooms"].append(invalid_manifest["rooms"][0])
        elif mutation == "duplicate_snapshot":
            duplicate = dict(invalid_manifest["snapshots"][0])
            duplicate["room_generation"] += 1
            invalid_manifest["snapshots"].append(duplicate)
        else:
            invalid_manifest["snapshots"] = []
        try:
            export_manifest.validate(invalid_manifest)
            validate_manifest_semantics(invalid_manifest)
        except (ValidationError, ValueError):
            continue
        raise AssertionError(f"{mutation} export manifest unexpectedly passed")
                    ...
                    if key in ID_KEYS:
                        ...
                        for identifier in values:
                            if identifier is None:
                                assert key == "commit_id", (
                                    f"{path}: only dry-run commit_id may be null"
                                )
                                continue
                            assert isinstance(identifier, str), (
                                f"{path}: identifier is not a string"
                            )
                            assert ID_RE.fullmatch(identifier), (
                                f"{path}: noncanonical ID {identifier}"
                            )
                    if key.endswith("_at"):
...
def test_obsidian_vault_exact_bytes_and_notes() -> None:
    ...
    assert not any(path.is_symlink() for path in all_paths), (
        "fixture tree cannot contain symbolic links"
    )
    actual_paths = sorted(
        path.relative_to(VAULT).as_posix() for path in all_paths if path.is_file()
    )
    assert actual_paths == expected_paths
    assert len({unicodedata.normalize("NFC", path).casefold() for path in actual_paths}) == len(
        actual_paths
    )
    ...
    assert (
        canonical_json_bytes(managed) + b"\n"
        == (VAULT / "state" / "managed-files.json").read_bytes()
    )
    assert (
        canonical_json_bytes(load_json(VAULT / "state" / "adapter.json")) + b"\n"
    ...
    for line in (
        (VAULT / "rooms" / "018f0f6e-7b6a-7c10-8af1-7f4c620b93c1" / "events" / "2026-08.ndjson")
        .read_bytes()
        .splitlines()
    ):
        assert canonical_json_bytes(json.loads(line)) == line
    ...
    note_paths = sorted(
        path for path in VAULT.rglob("*.md") if path.name not in {"README.md", "room.md"}
    )
    assert len(note_paths) == 4
    expected_revisions = [
        load_json(path) for path in sorted((ADAPTER_FIXTURES / "valid").glob("*-record.json"))
    ]
    ...
    assert authoritative == sorted(
        authoritative, key=lambda record: (record["record_id"], record["revision"])
    )
    assert authoritative == expected_revisions
    room_history = [
        record
        for record in authoritative
        if record["record_id"] == "018f0f70-9a8c-75dc-b6bf-dfe68dbe71bd"
    ]
    ...
    for path in note_paths:
        ...
        if record["kind"] == "persona":
            ...
            direction = (
                f"{record['scope']['source_persona_id']}--{record['scope']['target_persona_id']}"
            )
...
def test_required_security_and_contract_topics() -> None:
    ...
    topic_groups = {
        ...
        "conformance": (
            conformance,
            ["cross-room", "symlink/junction", "deterministic", "export secrets", "duplicate-key"],
        ),
    }
...
def test_adapter_acknowledges_authority_assigned_event_position() -> None:
    envelopes = load_json(ADAPTER_FIXTURES / "valid" / "operation-envelopes.json")
    append = next(item for item in envelopes if item["operation"] == "append_events")
    requested = {
        (event["event_id"], event["event_sequence"], event["content_digest"])
        for event in append["request"]["events"]
    }
    acknowledged = {
        (event["event_id"], event["event_sequence"], event["content_digest"])
        for event in append["response"]["assigned"]
    }
    assert acknowledged == requested, "sink must echo, not allocate or rewrite, room order"


def test_local_markdown_links() -> None:
    markdown_paths = [
        ROOT / "docs" / "adr" / "0004-self-hosted-memory-adapters.md",
        *sorted(MEMORY.rglob("*.md")),
    ]
    checked = 0
...
def main() -> int:
    tests = [
        ...
        test_adapter_acknowledges_authority_assigned_event_position,

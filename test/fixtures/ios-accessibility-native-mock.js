...
const database = {
  ...
  async executeBatch(call) {
    for (const statement of call.payload.statements) {
      ...
      if (statement.sqlId === "create_room") {
        state.room = { id: parameters[0], title: parameters[1], status: "active", generation: 0, inferenceMode: parameters[2], participants: [] };
        state.events = [];
      ...
      } else if (statement.sqlId === "save_local_draft") {
        state.draft = parameters[1];
      } else if (statement.sqlId === "delete_local_draft") {
        state.draft = "";
      }
    }
  ...
  async query(call) {
    ...
    if (sqlId === "room_list") return success(call, rows("room_summary_json", state.room ? [{ id: state.room.id, title: state.room.title, inferenceMode: state.room.inferenceMode, lastActivityOrder: state.events.length }] : []));
